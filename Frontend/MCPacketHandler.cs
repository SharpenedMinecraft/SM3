...
namespace Frontend
{
    public sealed class MCPacketHandler : IPacketHandler
    {
        ...
        public MCPacketHandler(ILogger<MCPacketHandler> logger, IPacketResolver resolver)
        {
            _resolver = resolver;
            _logger = logger;
        ...
        {
            var packet = _resolver.GetPacket(id, ctx, true);
            if (packet == null)
            {
            ...
            packet.Read(reader);
            packet.Process(_logger, ctx, packetQueue);
        }
    }
}